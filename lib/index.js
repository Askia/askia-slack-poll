...
app.post("/actions", bodyParser.urlencoded({extended: false}), (req, res) => {
  ...
  pollIdFrom(callback_id)
    ...
    .then(poll => teams.get(team.id).then(({token}) => [poll, token]))
    .then(([msg, token]) => web.chat.update({token, ...msg}))
    .then(handleResponse)
    .then(_ => res.status(200).end())
    .catch(err => sendError(res, {
      "channel": channel.id,
...
const showPoll = (team_id, user_id, channel_id, argv) => polls
  ...
  .then(poll => teams.get(team_id).then(({token}) => [poll, token]))
  .then(([poll, token]) =>  web.chat
    .postMessage({token, ...view.create(poll)})
    .then(handleResponse)
